...
func TestListCart(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			productListerMock: func(mc *minimock.Controller) ProductLister {
				...
				for i := 0; i < itemNum; i++ {
					mock.GetProductMock.When(ctx, cartItems[i].SKU).Then(products[i], nil)
				}
		...
		},
		{
			...
			productListerMock: func(mc *minimock.Controller) ProductLister {
				mock := mocks.NewProductListerMock(mc)
				mock.GetProductMock.When(ctx, cartItems[0].SKU).Then(model.ProductInfo{}, errListProduct)
		...
		},
		{
			...
			productListerMock: func(mc *minimock.Controller) ProductLister {
				...
				mock.GetProductMock.When(ctx, cartItems[itemNum-1].SKU).Then(model.ProductInfo{}, errListProduct)
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			domain := NewMockDomain(cfg, tt.checkoutRepoMock(mc), tt.productListerMock(mc))
